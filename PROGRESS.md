...
- **Data CSV and model.pkl are gitignored, not committed** (2026-07-11 decision, reversing the earlier default) — following prod practice: generated artifacts are regenerated via `data/generate_data.py` + `model/train_model.py`, not stored in version control. Anyone cloning the repo runs both scripts (fixed seed = 42, so output is reproducible) rather than pulling a stale binary blob.
